function single_unit_pstshs(ev_times, sp, flip_time, session_info, ops)
% 
% Plot event-aligned unit activity for all units in a session
nN = length(sp.cids);
...
save_folder = fullfile(ops.saveDir, 'PSTHs_goodTrials', animal, session);
...
for n = 1:nN
    try
        ...
        f = single_unit.plot_unit_psths(ev_times, st, flip_time, ops);
